...
#include "ui_zsekocontroldialog.h"
...
std::list<std::string> ZSekoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSekoControlDialog::listMints;

ZSekoControlDialog::ZSekoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSekoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSekoControlDialog::~ZSekoControlDialog()
{
    delete ui;
}

void ZSekoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSekoControlDialog::updateList()
{
...
void ZSekoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSekoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSeko_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSekoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSekoControlDialog::GetSelectedMints()
{
...
void ZSekoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
