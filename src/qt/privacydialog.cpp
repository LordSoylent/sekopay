                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSEKO ought to be enough for anybody." - Bill Gates, 2017
    ui->zSEKOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSEKOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSEKO->setEnabled(false);
        ui->pushButtonMintzSEKO->setToolTip(tr("zSEKO is currently disabled due to maintenance."));

        ui->pushButtonSpendzSEKO->setEnabled(false);
        ui->pushButtonSpendzSEKO->setToolTip(tr("zSEKO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSEKOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSEKO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSEKO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSEKO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSEKO in ") +
...
void PrivacyDialog::on_pushButtonSpendzSEKO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSEKO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSEKO
        sendzSEKO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSEKO
    sendzSEKO();
}

void PrivacyDialog::on_pushButtonZSekoControl_clicked()
{
    ZSekoControlDialog* zSekoControl = new ZSekoControlDialog(this);
    zSekoControl->setModel(walletModel);
    zSekoControl->exec();
}

void PrivacyDialog::setZSekoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSekoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSEKO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Seko Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSEKOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSEKO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SEKO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSEKO</b>";
    ...
    // use mints from zSeko selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSekoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSekoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zseko selector in case it was used
    ZSekoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSeko Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Seko, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSeko Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSEKO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSEKO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSEKO "));
    ui->labelzSEKOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSEKOSyncStatus->setVisible(fShow);
}
